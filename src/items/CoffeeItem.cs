...
namespace GameLab
{
    public class CoffeeItem(SpriteBatch sb, Vector2 position) : Item(sb, position)
    {
        ...
        {
            _enemyTexture = content.Load<Texture2D>("img/coffee_border");
        }

        public override void PickUp(Player player, GameTime gameTime)
        {
            player.GetSpeed(gameTime);
        }
